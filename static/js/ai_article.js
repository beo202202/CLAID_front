...
async function postArticle() {
    ...
    if (song_info == "" || voice == "" || song == null) {
        ...
        const response = await fetch(`${backend_base_url}/article/vocal/`, {
            method: 'POST',
            headers: {
        ...
        if (response.status == 201) {
            alert("작성완료!");
            window.location.replace('../ai_article.html');
...
async function getArticles() {
    const response = await fetch('http://127.0.0.1:8000/article/vocal/');
...
async function loadArticles() {
    ...
    const articleList = $("#ai_article_list");

    articles.forEach(article => {
        const newCol = $("<div>").addClass("ai_col");
        const newCard = $("<div>").addClass("ai_card").attr("id", article.id);
        newCol.append(newCard);

        const newCardBody = $("<div>").addClass("card_body").attr("onclick", `articleDetail(${article.id})`);
        newCard.append(newCardBody);

        const newCardTitle = $("<h4>").addClass("card_song_info").text("원곡정보: " + article.song_info);
        newCardBody.append(newCardTitle);

        const newCardPlay = $("<div>").addClass("card_play");
        newCard.append(newCardPlay);

        const newCardVoice = $("<div>").addClass("card_voice").text("목소리: " + article.voice);
