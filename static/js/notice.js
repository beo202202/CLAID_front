...
async function postArticle() {
    ...
    const article_image = document.getElementById("notice_article_image").files[0];
...
async function loadArticles() {
    ...
    articles.forEach(article => {
        const newListCol = $("<div>").addClass("list_col").attr("onclick", `articleDetail(${article.id})`);
        ...
        const newListUpdated = $("<li>").addClass("list_updated").text(timeago(article.updated_at));
        newListUl.append(newListUpdated);

        const newListNickname = $("<li>").addClass("list_nickname").text(article.user.nickname);
        newListUl.append(newListNickname);


        // const newListPlay = $("<li>").addClass("hits"); #조회수
        // newList.append(newListPlay);
